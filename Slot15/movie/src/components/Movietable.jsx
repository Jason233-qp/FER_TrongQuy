...
const MovieTable = ({ movies: filteredMovies }) => {
  const state = useMovieState();
  const { dispatch, confirmDelete } = useMovieDispatch();

  const { movies, genres, loading, movieToDelete, showDeleteModal } = state;

  // Nếu component nhận "movies" từ props → ưu tiên dùng cái đó
  const displayMovies = filteredMovies && filteredMovies.length ? filteredMovies : movies;
              ...
              displayMovies.map((movie) => {
                ...
                return (
                  <tr key={movie.id}>
                    <td>
                      <Image
                        src={movie.avatar || 'https://via.placeholder.com/50'}
                        alt={movie.title}
